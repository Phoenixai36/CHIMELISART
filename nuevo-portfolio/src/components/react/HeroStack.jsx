...
void main(){gl_Position=position;}`;
  ...
  const loop = (now) => {
    ...
    if (document.hidden) { animationFrameRef.current = requestAnimationFrame(loop); return; }
    if (now - lastFrameTime.current < frameInterval) { animationFrameRef.current = requestAnimationFrame(loop); return; }
...
const HeroStack = ({ trustBadge, headline, subtitle, buttons, className = "" }) => {
  ...
  return (
    <div className={`relative w-full h-screen overflow-hidden bg-black ${className}`}>
      <style>{`
            ...
            `}</style>
      <canvas ref={canvasRef} className="absolute inset-0 w-full h-full object-contain touch-none bg-[#1a120b] pointer-events-none md:pointer-events-auto" />
      ...
      <div className="absolute inset-0 z-10 flex flex-col items-center justify-center text-white">
        ...
        <div className="text-center space-y-6 max-w-5xl mx-auto px-4">
          <div className="space-y-4 w-full">
            ...
            {headline.line2 && (
              <h2 className="text-[3.5vw] md:text-5xl lg:text-6xl font-medium uppercase tracking-[0.15em] bg-gradient-to-r from-stone-200 via-stone-100 to-stone-300 bg-clip-text text-transparent animate-fade-in-up animation-delay-400 drop-shadow-[0_5px_15px_rgba(0,0,0,0.7)]">
                {headline.line2}
              </h2>
            )}
          </div>
