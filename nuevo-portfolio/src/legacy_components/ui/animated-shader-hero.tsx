...
const useShaderBackground = () => {
  ...
  const targetFps = 60; // Increased to 60 for smoothness, shader optimization will handle load
...
void main(){gl_Position=position;}`;
  ...
  const resize = () => {
    ...
    const dpr = Math.min(2, Math.max(1, 0.5 * window.devicePixelRatio));
  ...
  const loop = (now: number) => {
    ...
    // Performance optimization: Skip frames if not visible
    if (document.hidden) {
  ...
  useEffect(() => {
    ...
    const canvas = canvasRef.current;
    // Performance optimization: More conservative DPR for initial load smoothness
    // Use lower dpr for high resolution screens to avoid "laggy start"
    const isMobile = window.innerWidth < 768;
    const dpr = isMobile ? Math.min(0.8, window.devicePixelRatio) : Math.min(0.6, window.devicePixelRatio);
...
const Hero: React.FC<HeroProps> = ({
  ...
  return (
    <div className={`relative w-full h-screen overflow-hidden bg-black ${className}`}>
      ...
      <canvas
        ref={canvasRef}
        className="absolute inset-0 w-full h-full object-contain touch-none bg-[#1a120b] pointer-events-none md:pointer-events-auto"
      />
      
      {/* Subtle overlay for better text readability */}
      <div className="absolute inset-0 bg-gradient-to-b from-transparent via-black/20 to-black/30 pointer-events-none" />
      ...
      <div className="absolute inset-0 z-10 flex flex-col items-center justify-center text-white">
        ...
        <div className="text-center space-y-6 max-w-5xl mx-auto px-4">
          ...
          <div className="space-y-4 w-full">
            <h1 className="text-[10.2vw] md:text-7xl lg:text-8xl font-bold bg-gradient-to-r from-orange-300 via-yellow-400 to-amber-300 bg-clip-text text-transparent animate-fade-in-up animation-delay-200 drop-shadow-[0_10px_20px_rgba(0,0,0,0.8)] tracking-[-0.1em] leading-none">
              {headline.line1}
            </h1>
            {headline.line2 && (
              <h2 className="text-[3.5vw] md:text-5xl lg:text-6xl font-medium uppercase tracking-[0.15em] bg-gradient-to-r from-stone-200 via-stone-100 to-stone-300 bg-clip-text text-transparent animate-fade-in-up animation-delay-400 drop-shadow-[0_5px_15px_rgba(0,0,0,0.7)] font-heading">
                {headline.line2}
              </h2>
            )}
          </div>
          ...
          <div className="max-w-2xl mx-auto animate-fade-in-up animation-delay-600">
            <div className="text-white/90 leading-relaxed drop-shadow-[0_2px_8px_rgba(0,0,0,0.8)]">
              {subtitle}
            </div>
          </div>
...
void main(void) {
	...
	uv*=1.-.3*(sin(T*.2)*.5+.5);
	// Reduced iterations from 12 to 8 for better performance (laggy start fix)
	for (float i=1.; i<8.; i++) {
	...
	// CHIMELISART color scheme - tuned to be richer and less "too yellow"
    // Use a darker, more cinematic base background mixing
	col=mix(col,vec3(bg*.15, bg*.1, bg*.05),length(uv));
	
	// Add subtle copper/amber tint but balance with some darkness/void
	O=vec4(col,1);
}`;
